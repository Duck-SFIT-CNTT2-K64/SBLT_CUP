    ...
    <div className="py-12 px-6 lg:px-8 max-w-[1280px] max-w-5xl">
      <RevealOnScroll>
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-2xl font-bold text-[#f5f5f5] sblt-heading">Quản lý Thông báo</h1>
            <p className="text-[#888] mt-1">Tạo và quản lý thông báo cho tuyển thủ</p>
          </div>
      ...
      <RevealOnScroll>
        <div className="space-y-4">
          {announcements.map((a) => (
            <Card key={a.id} hover={false} className="p-5 hover:border-[#dc2626]/60 hover:-translate-y-0.5 hover:shadow-[0_0_24px_rgba(220,38,38,0.15)]">
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    <Badge variant="default">{TYPE_LABELS[a.type] || a.type}</Badge>
                    {a.tournament && <span className="text-xs text-[#dc2626]">{a.tournament.name}</span>}
                  </div>
                  <h2 className="text-lg font-semibold text-[#f5f5f5] mb-1">{a.title}</h2>
                  <p className="text-[#888] text-sm line-clamp-2">{a.content}</p>
                  <div className="mt-2 text-xs text-[#222]">{new Date(a.createdAt).toLocaleString("vi-VN")}</div>
                </div>
                <div className="flex gap-1">
                  <button onClick={() => handleEdit(a)} className="p-2 text-[#888] hover:text-white hover:bg-[#222] rounded-lg transition-colors"><Edit className="h-4 w-4" /></button>
                  <button onClick={() => handleDelete(a.id)} className="p-2 text-[#888] hover:text-red-400 hover:bg-[#222] rounded-lg transition-colors"><Trash2 className="h-4 w-4" /></button>
                </div>
              </div>
            </Card>
          ))}
      ...
      {announcements.length === 0 && (
        <div className="text-center py-12"><Bell className="h-12 w-12 text-[#222] mx-auto mb-4" /><p className="text-[#888]">Chưa có thông báo nào</p></div>
      )}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <Card hover={false} className="p-6 w-full max-w-lg mx-4">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-[#f5f5f5]">{editingId ? "Sửa thông báo" : "Tạo thông báo mới"}</h2>
              <button onClick={() => setShowModal(false)} className="text-[#888] hover:text-white"><X className="h-5 w-5" /></button>
            </div>
            <form onSubmit={handleSubmit}>
              <div className="mb-4">
                <label className="block text-sm font-medium text-[#888] mb-1.5">Tiêu đề</label>
                <input type="text" value={formData.title} onChange={(e) => setFormData((p) => ({ ...p, title: e.target.value }))} className={inputClass} required />
              </div>
              <div className="mb-4">
                <label className="block text-sm font-medium text-[#888] mb-1.5">Loại thông báo</label>
              ...
              <div className="mb-6">
                <label className="block text-sm font-medium text-[#888] mb-1.5">Nội dung</label>
              ...
              <div className="flex justify-end gap-3">
                <button type="button" onClick={() => setShowModal(false)} className="px-4 py-2 text-[#888] hover:text-white transition-colors">Hủy</button>
