...
export default function ProfilePage() {
  ...
  const inputClass = "w-full px-4 py-2.5 bg-sblt-dark border border-sblt-border rounded-xl text-white placeholder:text-sblt-border focus:outline-none focus:ring-2 focus:ring-sblt-red focus:border-transparent transition-shadow";

  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold flex items-center gap-3 text-white">
          <User className="h-7 w-7 text-sblt-red" /> Hồ sơ cá nhân
        </h1>
        <p className="text-sblt-muted mt-2">Cập nhật thông tin thi đấu của bạn</p>
      ...
      <Card hover={false} className="p-6">
        <form onSubmit={handleSubmit}>
          <div className="mb-5">
            <label className="block text-sm font-medium text-sblt-muted mb-1.5">Tên ingame (TFT) *</label>
            <input type="text" value={profile.ign} onChange={(e) => setProfile((p) => ({ ...p, ign: e.target.value }))} className={inputClass} required />
            <p className="text-xs text-sblt-border mt-1">Tên này sẽ được sử dụng trong suốt giải đấu. Liên hệ BTC nếu cần đổi.</p>
          </div>
          <div className="mb-5">
            <label className="block text-sm font-medium text-sblt-muted mb-1.5">Rank hiện tại</label>
          ...
          <div className="mb-5">
            <label className="block text-sm font-medium text-sblt-muted mb-1.5">Discord username</label>
          ...
          <div className="mb-6">
            <label className="block text-sm font-medium text-sblt-muted mb-1.5">Số điện thoại</label>
